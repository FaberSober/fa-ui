...
| 页面性能 | 评估行级 StudentModal | 大 pageSize 下按需使用页面级单例弹窗 | 先验证后决定 | ✅评估完成，暂不改造 |
| 高级查询 | 核对场景与条件展示语义 | 确认 `sceneId` 与 `conditionList` 是否需要同步 | 先验证后决定 | ✅评估完成，不改造 |
| 学生弹窗 | 核对关闭后的表单残留 | 验证取消后再次打开是否保留旧值 | 先验证后决定 | ✅评估完成，不改造 |
| 表格初始化 | 延迟非必要配置请求 | 评估场景列表、列配置请求的懒加载或缓存 | 执行开发 | ✅已完成 |
| 表格渲染 | 列定义与模块依赖清理 | 按需 memo 化列生成，清理循环/无用导入和重复 UUID 计算 | 执行开发 | ✅已完成 |
...
- 当前学生表默认 `pageSize` 为 20，关闭状态的 `DragModal` 不生成弹窗 DOM；现有代码和页面规模未提供需要改造的性能证据，暂时保留行级弹窗。
- 若后续将该页面稳定用于 100 条以上的 pageSize，或实际性能采样显示行级实例成为瓶颈，再实施页面级单例弹窗。

### FBT-09 核对场景与条件展示语义

- 位置：`ComplexQuery.tsx`、`SceneDropMenu.tsx`、`ConditionQueryModal.tsx`。
- 后端 `BaseBiz.processSceneId` 会将场景保存的条件追加到请求已有的 `conditionList`；`WrapperUtils` 对每个条件分组使用 `AND` 组合，分组内部再按分组类型处理 `AND`/`OR`，因此 `sceneId` 与 `conditionList` 会同时生效。
- `useTableQueryParams` 会同时发送两个参数，`ComplexQuery` 分别展示当前场景名称和高级查询条件摘要，前端展示与实际查询语义一致，无需增加场景条件回显或切换场景时清理条件状态。
...
- 已验证编辑取消、新增取消、切换编辑记录以及不同关闭方式后再次打开，所有字段均符合预期，未发现旧值残留；现有 `setFieldsValue(getInitialValues())` 逻辑足够，无需增加 `resetFields`。
...
- `SceneDropMenu` 不再在初始化时加载场景列表，改为首次展开下拉菜单时加载；请求在组件生命周期内复用，管理弹窗通过同一加载器获取列表，保存、删除或排序后强制刷新，失败时允许下次重试。
- `TableColConfigModal` 保留初始化加载列配置，以确保用户已保存的隐藏列在首屏生效；初始化请求与打开抽屉复用同一个加载状态，避免重复调用 `getOne`，保存、重置和 `biz` 切换时同步更新或重新加载配置。

### FBT-12 列定义与模块依赖清理

- 位置：学生表 `index.tsx`、`BaseBizTable.tsx`、`base-table/utils.tsx`。
- 学生表列定义改为 `useMemo`，依赖排序、字典数据和操作回调，避免非相关渲染重复创建列定义。
- `BaseBizTable` 将列配置合并和横向滚动宽度计算放入 `useMemo`，依赖 `config`、`columns`、`showRowNum`，不改变原有列过滤和合并语义。
- `utils.tsx` 改为直接导入 `FaberTable` 类型，移除对自身 `base-table/index` barrel 的无用运行时依赖，避免循环依赖。
- 当前 `base-table` 路径没有 UUID 初始化逻辑，因此不修改无关模块。
