...
namespace FaberTable {
  ...
  export interface ColumnsProp<T> extends ColumnProps<T> {
    ...
    /** 是否必须 */
    tcRequired?: boolean;
    /** 是否默认勾选展示本字段，true-默认勾选展示/false-默认不展示。不填写默认为true */
    tcChecked?: boolean;
    /** 高级查询-自定义条件查询筛选值自定义录入组件-单值录入组件-方法定义 */
    tcCondComponent?: (props: TcCondProp) => JSX.Element;
    // tcCondComponentElement?: FunctionComponent; // 使用FunctionComponent类型dev保存，build打包无法运行
    /** 高级查询-自定义条件查询筛选值自定义录入组件-单值录入组件-组件定义 */
    tcCondComponentElement?: any;
    /** 高级查询-自定义条件查询筛选值自定义录入组件-双值录入组件-方法定义 */
    tcCondBetweenComponent?: (props: TcCondBetweenProps) => JSX.Element;
    /** 高级查询-自定义条件查询筛选值自定义录入组件-双值录入组件-组件定义 */
    tcCondBetweenComponentElement?: any;
    /** 自定义类型:menu操作菜单/ */
    tcType?: string | undefined;
    /** 自定义条件查询-是否隐藏 */
    tcConditionHide?: boolean;
    /** 自定义条件查询-标签 */
    tcLabel?: string;
  }
