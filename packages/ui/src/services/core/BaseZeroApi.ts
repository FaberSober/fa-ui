...
export default class BaseZeroApi {
  ...
  protected get = <E>(api: string, params?: any, config?: AxiosRequestConfig) =>
    requestGet<E>(`${this.apiPrefix}/${this.apiModal}/${api}`, { ...config, params }).catch(e => {console.log('get', e)});

  protected delete = <E>(api: string, config?: AxiosRequestConfig) =>
    requestDelete<E>(`${this.apiPrefix}/${this.apiModal}/${api}`, config).catch(e => {console.log('delete', e)});

  protected post = <E>(api: string, body: object, config?: AxiosRequestConfig) =>
    requestPost<E>(`${this.apiPrefix}/${this.apiModal}/${api}`, body, config).catch(e => {console.log('post', e)});
  ...
  protected put = <E>(api: string, body: object, config?: AxiosRequestConfig) =>
    requestPut<E>(`${this.apiPrefix}/${this.apiModal}/${api}`, body, config).catch(e => {console.log('put', e)});

  protected download = (api: string, body: object, config?: AxiosRequestConfig) =>
    requestDownload(`${this.apiPrefix}/${this.apiModal}/${api}`, body, config).catch(e => {console.log('download', e)});
